...
/**
 * Carries trusted uniqueness facts through the plan for native execution.
 *
 * The facts are derived from LogicalProperties, which in turn only use
 * connector/table constraints that the planner already trusts (for example
 * enabled or RELY unique/primary-key constraints). This rule does not change
 * join semantics or shape; it annotates join keys so downstream engines can
 * expose metrics and later pick specialized physical implementations.
 */
        ...
        implements Rule<JoinNode>
{
    ...
    public Result apply(JoinNode joinNode, Captures captures, Context context)
    {
        ...
        if (joinNode.isLeftKeysUnique() == leftKeysUnique && joinNode.isRightKeysUnique() == rightKeysUnique) {
            return Result.empty();
        }

        return Result.ofPlanNode(joinNode.withUniqueKeys(leftKeysUnique, rightKeysUnique));
    }
