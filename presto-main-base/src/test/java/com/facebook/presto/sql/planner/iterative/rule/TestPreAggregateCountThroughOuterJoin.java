        ...
        extends BaseRuleTest
{
    ...
    public void testUsesCountAllForTrustedNotNullInnerColumn()
    {
        tester().assertThat(new PreAggregateCountThroughOuterJoin(getMetadata().getFunctionAndTypeManager()))
                .setSystemProperty(PUSH_AGGREGATION_THROUGH_JOIN, "true")
                .on(TestPreAggregateCountThroughOuterJoin::countOverLeftJoinWithTrustedNotNullInnerColumn)
                .matches(node(ProjectNode.class,
                        node(AggregationNode.class,
                                node(JoinNode.class,
                                        any(),
    ...
    @Test
    public void testDoesNotUseCountAllWhenConstraintExploitationDisabled()
    {
        tester().assertThat(new PreAggregateCountThroughOuterJoin(getMetadata().getFunctionAndTypeManager()))
                .setSystemProperty(PUSH_AGGREGATION_THROUGH_JOIN, "true")
                ...
                .matches(node(ProjectNode.class,
                        node(AggregationNode.class,
                                node(JoinNode.class,
                                        any(),
                                        aggregation(
                                                ImmutableMap.of("pre_count", functionCall("count", false, ImmutableList.of(anySymbol()))),
                                                any())))));
    }
    ...
    @Test
    public void testDoesNotFireWhenGroupingByInnerVariable()
    {
        tester().assertThat(new PreAggregateCountThroughOuterJoin(getMetadata().getFunctionAndTypeManager()))
                .setSystemProperty(PUSH_AGGREGATION_THROUGH_JOIN, "true")
                .on(p -> {
                    ...
                    return p.aggregation(aggregation -> aggregation
                            .singleGroupingSet(outerKey, innerValue)
                            .addAggregation(count, p.rowExpression("count(inner_value)"))
                            .source(p.join(
                                    LEFT,
                                    p.values(outerKey),
    ...
    @Test
    public void testDoesNotFireForJoinWithResidualFilter()
    {
        tester().assertThat(new PreAggregateCountThroughOuterJoin(getMetadata().getFunctionAndTypeManager()))
                .setSystemProperty(PUSH_AGGREGATION_THROUGH_JOIN, "true")
                .on(p -> {
                    ...
                    return p.aggregation(aggregation -> aggregation
                            ...
                            .source(p.join(
                                    ...
                                    p.values(innerKey, innerValue),
                                    ImmutableList.of(new EquiJoinClause(outerKey, innerKey)),
                                    ImmutableList.of(outerKey, outerValue, innerKey, innerValue),
                                    Optional.of(p.rowExpression("outer_value > inner_value")))));
                })
                .doesNotFire();
    }
    ...
    private static PlanNode countOverLeftJoinWithTrustedNotNullInnerColumn(PlanBuilder p)
    {
        VariableReferenceExpression outerKey = p.variable("outer_key", BIGINT);
        VariableReferenceExpression innerKey = p.variable("inner_key", BIGINT);
        VariableReferenceExpression innerValue = p.variable("inner_value", BIGINT);
