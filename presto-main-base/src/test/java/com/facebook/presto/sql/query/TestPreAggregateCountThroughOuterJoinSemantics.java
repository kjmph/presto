...
public class TestPreAggregateCountThroughOuterJoinSemantics
{
    ...
    @Test
    public void testLeftJoinCountWithCompoundNullableJoinKeys()
    {
        try (QueryAssertions assertions = new QueryAssertions(ImmutableMap.of(PUSH_AGGREGATION_THROUGH_JOIN, "true"))) {
            assertQueryAndPreAggregatedOuterJoin(
                    assertions,
                    "WITH " +
                            "outer_relation(group_key, join_key_1, join_key_2) AS (VALUES " +
                            "    (1, 10, 100), (1, 10, 100), (1, 10, NULL), (2, NULL, 200), (3, 30, 300)), " +
                            "inner_relation(join_key_1, join_key_2, value) AS (VALUES " +
                            "    (10, 100, 'a'), (10, 100, 'b'), (10, NULL, 'x'), (NULL, 200, 'y'), (30, 300, NULL), (40, 400, 'z')) " +
                            "SELECT group_key, count(value) " +
                            "FROM outer_relation " +
                            "LEFT JOIN inner_relation ON " +
                            "    outer_relation.join_key_1 = inner_relation.join_key_1 AND " +
                            "    outer_relation.join_key_2 = inner_relation.join_key_2 " +
                            "GROUP BY group_key",
                    "VALUES (1, BIGINT '4'), (2, BIGINT '0'), (3, BIGINT '0')",
                    LEFT);
        }
    }

    @Test
    public void testRightJoinCountWithCompoundNullableJoinKeys()
    {
        try (QueryAssertions assertions = new QueryAssertions(ImmutableMap.of(PUSH_AGGREGATION_THROUGH_JOIN, "true"))) {
            assertQueryAndPreAggregatedOuterJoin(
                    assertions,
                    "WITH " +
                            ...
                            "SELECT group_key, count(value) " +
                            "FROM inner_relation " +
                            "RIGHT JOIN outer_relation ON " +
                            "    inner_relation.join_key_1 = outer_relation.join_key_1 AND " +
                            "    inner_relation.join_key_2 = outer_relation.join_key_2 " +
                            "GROUP BY group_key",
                    "VALUES (1, BIGINT '4'), (2, BIGINT '0'), (3, BIGINT '0')",
                    RIGHT);
        }
