...
Set this property to ``false`` to disable the optimization for the current session, for
example as an operational kill switch or when comparing plans during performance testing.

The corresponding configuration property is :ref:`admin/properties:\`\`optimizer.push-aggregation-through-unique-lookup-join\`\``.
